...
(deftest nist-test
  (testing "nist test data: \"http://www.nsrl.nist.gov/testdata/\""
    (is (= (->hex (digest (message-digest "SHA-1" "UTF-8") "abc"))
           "A9993E364706816ABA3E25717850C26C9CD0D89D"))
    (is (= (->hex (digest (message-digest "SHA-1" "UTF-8") 
                    "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq"))
           "84983E441C3BD26EBAAE4AA1F95129E5E54670F1"))
    (is (= (->hex (apply digest (message-digest "SHA-1" "UTF-8")
                                (repeat 1000000 "a")))
           "34AA973CD4C4DAA4F61EEB2BDBAD27316534016F"))

    (is (= (->hex (digest (message-digest "MD5" "UTF-8") "abc"))
           "900150983CD24FB0D6963F7D28E17F72"))
    (is (= (->hex (digest (message-digest "MD5" "UTF-8")
                    "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq"))
           "8215EF0796A20BCAAAE116D3876C664A"))
    (is (= (->hex (apply digest (message-digest "MD5" "UTF-8")
                                (repeat 1000000 "a")))
           "7707D6AE4E027C70EEA2A935C2296F21"))

    (is (= (->hex (digest (message-digest "SHA-256" "UTF-8") "abc"))
           "BA7816BF8F01CFEA414140DE5DAE2223B00361A396177A9CB410FF61F20015AD"))
    (is (= (->hex (digest (message-digest "SHA-256" "UTF-8")
                    "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq"))
           "248D6A61D20638B8E5C026930C3E6039A33CE45964FF2167F6ECEDD419DB06C1"))
    (is (= (->hex (apply digest (message-digest "SHA-256" "UTF-8")
                                (repeat 1000000 "a")))
...
(deftest wiki-test
  (testing "wiki test data: \"http://en.wikipedia.org/wiki/SHA-1\""
    ...
    (is (= (->hex (digest (message-digest "SHA-512" "UTF-8") 
                    "The quick brown fox jumps over the lazy dog"))
